...
let mData = moviesData


function movietemplate(movie) {
    return `
    <div class="movie-data">
        <img alt="${movie.title}" class="movie-poster" src="${movie.posterUrl}">
        <ul class="movie-atributes">
            <li class="movie-name">${movie.title}</li>
            <li class="movie-year">Year Released ${movie.year}</li>
            <li class="movie-director"><strong>Director:<strong> ${movie.director}.</li>
            <li class="movie-genre"> Genres: ${movie.genres.join(", ")}</li>
            <li class="movie-actors">Actors: ${movie.actors}</li>
...
<p class="text"> These (${moviesData.length}) movies were added recently come back later for more</p>
`;

document.getElementById("head").innerHTML = `
<h3 class="text"> movies (${moviesData.length} results) </h3>
`;
...
search.addEventListener("keyup", (e) => {
    ...
    const filteredCharacters = mData.filter( character => {
        return (
            character.title.includes(searchString)  || 
            character.genres.includes(searchString)
        );
        
    });
    document.getElementById("head").innerHTML = `
<h3 class="text"> movies results (${filteredCharacters.length} ) </h3>
`;

    document.getElementById("app").innerHTML = `
${filteredCharacters.map(movietemplate).join("")}
`;
});
